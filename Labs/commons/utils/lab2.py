...
def test_KNN_Clasificacion(func):
    ...
    lr1, dr1 =  func(k=5,Xtrain = np.random.rand(10,2),  Ytrain= np.random.choice(2,10), Xtest= np.random.rand(3,2))
    lr2, dr2 =  func(k=5,Xtrain = np.random.rand(10,2),  Ytrain= np.random.choice(2,10), Xtest= np.random.rand(3,2))
    r2 =  func(k=2, Xtrain=xtrains, Ytrain = ytrains, Xtest = xtests)
    r3 =  func(k=3,Xtrain=xtrains, Ytrain = ytrains, Xtest= xtests)
    r5 =  func(k=5,Xtrain=xtrains, Ytrain  = ytrains, Xtest= xtests)
...
def test_train_test_split_fix(func):
    x1,x2,y1,y2 = func(X = np.random.rand(10,2),Y=np.random.choice(2,10))
    x11,x21,y11,y21 = func(X =np.random.rand(50,3),Y= np.random.choice(2,50))
    ...
    code_to_look = [['test_size=', 'train_test_split', 'random_state=0'],
                    ['train_size=', 'train_test_split', 'random_state=0'],
                    ['test_size=', 'train_size=', 'train_test_split', 'random_state=0']
                    ]
...
def test_exp(func):
    ...
    res = ut.test_experimento_oneset(func,  shape_val=(len(ks), 2), 
                                    col_error = ['error en conjunto prueba'],
                                    col_val=['k-vecinos', 'error en conjunto prueba'],
                                    X = xx, Y=yy,
                                    ks = ks)
...
def test_parzenClass(func):
    ...
    l1,f1 =  func(h=0.1, Xtrain=xtrains, Ytrain=ytrains,Xtest=xtests)
    l2, f2 =  func(h=1,  Xtrain=xtrains, Ytrain=ytrains,Xtest=xtests)
    l3, f3 =  func(h=10,  Xtrain=xtrains, Ytrain=ytrains,Xtest=xtests)
...
def test_parzen_exp(func):
    ...
    res = ut.test_experimento_oneset(func,  shape_val=(len(hs), 2), 
                                    col_error = None,
                                    col_val=['valor h', 'error en conjunto prueba'],
                                    X = xx, Y=yy,
                                    hs = hs)
